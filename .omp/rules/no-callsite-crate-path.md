...
condition:
  ...
  - '(?m)^(?![ \t]*(pub(\([^)]*\))?[ \t]+)?use[ \t]+).*\b(zerolaunch_plugin_api|zerolaunch_plugin_protocol|zerolaunch_plugin_host|zerolaunch_plugin_sdk_rust)::'
scope:
  - tool:edit(**/*.rs)
  - tool:write(**/*.rs)
  - tool:ast_edit
...
在 Rust 代码中，**禁止**在调用点（表达式、参数、返回值、impl 等位置）书写 `crate::...` 或外部 crate（`zerolaunch_plugin_api::`、`zerolaunch_plugin_protocol::` 等）的完整限定路径。
类型、函数、枚举及枚举变体、宏**必须**在文件开头的 `use` 语句中导入，调用点使用短名。

## 原因

- 调用点全限定路径与 `use` 导入重复，同一类型在文件内出现两种引用形态，读代码需两处对照
- 模块路径变更（重构、移动模块、crate 版本升级）时所有调用点必须同步修改，`use` 集中在一处则只需改文件头
